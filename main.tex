...
\begin{itemize}
  ...
  \item \textbf{GeoGS: Geospatial Gaussian Splatting for Robust 3D Reconstruction from Sparse Satellite \mbox{Imagery}}. Han-Gyeol Kim$^*$, \textbf{Seonghyeon Yun}$^*$, JaeWan Park, Darongsae Kwon. \emph{CVPR 2026 EarthVision Workshop}. ($^*$ equal contribution)
\end{itemize}
...
\section*{Projects}
\begin{itemize}
  \item \textbf{Crypto-Market Quantitative Research}
        \begin{itemize}
          \item \textbf{Bittensor SN64 Mining Strategy}: dTAO emission modeling and single-GPU entry economics for the Chutes LLM-inference subnet.
          \item \textbf{Meteora DAMM v2 LP Rotation (Solana)}: Smoothed fee-yield ranking with periodic capital rotation across DeFi liquidity pools.
          \item \textbf{Aave / Morpho Liquidation Bot (Base)}: Health-factor monitoring with flash-loan execution.
          \item \textbf{MEXC New-Listing Pump Capture}: Post-listing entry backtest across CeFi spot pairs.
          \item \textbf{Pump.fun Meme-Coin ML Scanner (Solana)}: Real-time launch scanner with an ML paper trader.
        \end{itemize}
  \item \textbf{Parallel ARC-AGI Solver via Transductive LLMs and Rule-Based CPU Search} \\
        Team project, SNU Basics of Deep Learning (Spring 2025). Solving an easier subset of the ARC-AGI benchmark --- abstract reasoning tasks that ask the solver to infer a grid-transformation rule from a few input--output examples. Hybrid GPU--CPU system: transductive Llama-3.1-8B (LoRA + Test-Time Training) running concurrently with rule-based hard-coded solutions.
